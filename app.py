...
# =========================
# Sidebar Parametersimport streamlit as st
import pandas as pd
import requests
import numpy as np
import plotly.graph_objects as go
...
# Data Fetching
# -------------------------
def get_top_alts_safe(limit=30):
    """Fetch top altcoins data from CoinGecko"""
    ...
    try:
        ...
        st.error(f"Error fetching altcoin data: {e}")
        return pd.DataFrame()

def get_btc_data(days=365*5):
...
st.header("₿ Bitcoin Price & Resistance Levels")

btc_df = get_btc_data(365)
if not btc_df.empty:
    resistance_levels = [114000, 120000, 123000]  # current resistances only

    fig_btc = go.Figure()
    fig_btc.add_trace(go.Scatter(
        x=btc_df["Date"], y=btc_df["Price"],
        mode="lines", name="BTC Price", line=dict(color="blue")
    ))

    for lvl in resistance_levels:
        fig_btc.add_hline(
            y=lvl, line=dict(color="red", dash="dash"),
            annotation_text=f"Resistance {lvl:,}", annotation_position="top right"
        )

    fig_btc.update_layout(
        title="BTC Price with Current Resistance Levels",
...
st.header("📐 Bitcoin Power Law Chart")

btc_df_long = get_btc_data(365*5)
if not btc_df_long.empty:
    btc_df_long["Days"] = (btc_df_long["Date"] - btc_df_long["Date"].min()).dt.days + 1
    btc_df_long["LogDays"] = np.log10(btc_df_long["Days"])
    btc_df_long["LogPrice"] = np.log10(btc_df_long["Price"])

    # Linear regression on log-log scale
    coeffs = np.polyfit(btc_df_long["LogDays"], btc_df_long["LogPrice"], 1)
    btc_df_long["PowerLawFit"] = 10**(coeffs[0]*btc_df_long["LogDays"] + coeffs[1])

    fig_power = go.Figure()
    fig_power.add_trace(go.Scatter(
        x=btc_df_long["Date"], y=btc_df_long["Price"],
        mode="lines", name="BTC Price", line=dict(color="blue")
    ))
    fig_power.add_trace(go.Scatter(
        x=btc_df_long["Date"], y=btc_df_long["PowerLawFit"],
        mode="lines", name="Power Law Model", line=dict(color="orange", dash="dot")
    ))
    fig_power.update_yaxes(type="log")
    fig_power.update_layout(
        title="Bitcoin Power Law Model (Log Scale)",
        xaxis_title="Date", yaxis_title="Price (USD, log)",
        height=500
    )
    st.plotly_chart(fig_power, use_container_width=True)
else:
    st.warning("BTC long-term data unavailable.")

# --- Altcoin Rotation Treemap ---
st.header("🔥 Altcoin Rotation Treemap")

alt_df = get_top_alts_safe(30)
if not alt_df.empty:
    metric = st.selectbox(
        "Select performance metric for coloring:",
        options=["7d %", "24h %", "Mkt Cap ($B)"],
        index=0
    )
    ...
    fig_treemap = go.Figure(go.Treemap(
        ...
        marker=dict(
            colors=alt_df[metric],
            colorscale="RdYlGn" if metric in ["7d %", "24h %"] else "Blues",
            cmid=0 if metric in ["7d %", "24h %"] else None
        ),
        hovertemplate=(
            "<b>%{label}</b><br>"
            ...
            "7d: %{customdata[2]:.2f}%<extra></extra>"
        ),
        customdata=alt_df[["Price ($)", "24h %", "7d %"]].values
    ))
    fig_treemap.update_layout(
        margin=dict(t=50, l=25, r=25, b=25),
        title=f"Altcoin Rotation by Market Cap & {metric}"
    )
    st.plotly_chart(fig_treemap, use_container_width=True)
else:
    st.warning("Altcoin data unavailable.")
# =========================
